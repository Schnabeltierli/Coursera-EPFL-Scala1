...
object Huffman {
  ...
  // Part 1: Basics
    ...
    case Fork(l1, r1, c1, w) => w
    case Leaf(c1, w) => w
  } // tree match ...
  
    def chars(tree: CodeTree): List[Char] = tree match {
      case Fork(l1, r1, chars, w) => chars
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def travstep(remain: CodeTree, bits: List[Bit]): List[Char] = remain match {
        case Leaf(c, _) if bits.isEmpty => List(c)
        case Leaf(c, _) => c :: travstep(tree, bits)
        case Fork(left, right, _, _) if bits.head == 0 => travstep(left, bits.tail)
        case Fork(left, right, _, _) => travstep(right, bits.tail)
      }
